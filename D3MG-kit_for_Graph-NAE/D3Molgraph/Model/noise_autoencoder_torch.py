...
class Torch_Noise_AE(nn.Module):
    def __init__(self,inputdim,latentdim,noise_percent):
        super(Torch_Noise_AE,self).__init__()
        self.latentdim = latentdim
        self.noise_percent=noise_percent 
        self.econv1=nn.Conv2d(1,128,3,(1,1))
        self.econv2=nn.Conv2d(1,256,3,(1,1))
    ...
    def evaluate_molgraphrmsd(self,MGset,with_noise=True):
        ...
        for mol in pbar:
            try:
                ...
                totalrms.append(rms)
                write_xyz(f'{self.trainingsteps}/model_{self.trainingsteps}_id_{mnum}_decode.xyz',mol.atoms,reverse_coord2,rms)
                write_xyz(f'{self.trainingsteps}/model_{self.trainingsteps}_id_{mnum}_original.xyz',mol.atoms,reverse_coord1,rms)                
                pbar.set_description(f"RMSD: {rms:0.4f}")
                mnum+=1  
            except:
                pass
